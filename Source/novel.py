...
def create_enc_df(level, graph, remove_carbon_neighbors=True, element_alphabet=['C', 'N', 'O', 'S']):
    ...
    enc_df.index = ['L' + str(i[0]) + ' - ' + i[1] for i in
                    cartesian((np.arange(1, level + 1), np.array(element_alphabet)))]
...
def shift_padding(enc_df, col_nr, element_alphabet=['C', 'N', 'O', 'S']):
    ...
    row_names = ['L' + str(i[0]) + ' - ' + i[1] for i in
                 cartesian((np.arange(1, level + 1), np.array(element_alphabet)))]
...
def generate_imgs_from_encoding(normalized_encoding, print_progress=False, foldername=None, level=2,
    ...
    for name, encoding in normalized_encoding.items():
        ...
        subdir_name = os.path.join(dir_name, 'novel1_level_' + str(level) + '_' + alphabet_mode)
...
def csv_export_novel1(normalized_encoding, output_path='encoding.csv'):
    """
...
def novel1_encode(smiles_list, level, generate_imgs=False, alphabet_mode='without_hydrogen', print_progress=False,
                  output_path=None, foldername_encoding_vis='./Novel_Encodings_Visualisation'):
...
def main():
    program_name = 'novel'
    program_description = '''find cool acronym'''  # TODO
    ...
    output_dir_name = "Novel_Encodings"
    ...
    output_path = os.path.join(output_dir, 'novel1_level_' + str(level) + '_' + alphabet_mode + '.csv')
    ...
    novel1_encode(smiles_list, level=level, generate_imgs=generate_imgs, alphabet_mode=alphabet_mode,
